...
export const NOT_FOUND_MESSAGE = "No Excel file found. Please place the latest Excel file in the configured folder.";

// Where to read the Excel export from. Checked in order:
//   1. PQ_DATA_FOLDER environment variable (e.g. PQ_DATA_FOLDER=D:\PQData)
//   2. "dataFolder" in pq.config.json at the project root (easiest to edit by hand)
...
export function resolveExcelDir(projectRoot) {
  const fromEnv = process.env.PQ_DATA_FOLDER && process.env.PQ_DATA_FOLDER.trim();
  if (fromEnv) return path.isAbsolute(fromEnv) ? fromEnv : path.join(projectRoot, fromEnv);

  try {
    const cfg = JSON.parse(fs.readFileSync(path.join(projectRoot, "pq.config.json"), "utf8"));
...
export function handleExcelFile(req, res, excelDir) {
  ...
  try {
    ...
    return sendJson(res, 500, { error: "Unable to read the Excel file. Please check the file and try again." });
  }
}
