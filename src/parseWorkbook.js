...
// This is a demo build: real party/customer names from the source workbook are
// never shown. Every distinct name across both the Party and Customer columns is
// replaced with a generic "Customer N" label, numbered by total balance qty
// (largest first) so the relative sizes on screen still read as meaningful.
function anonymizeNames(rows) {
  const totals = {};
  rows.forEach((r) => {
    if (r.p !== "—") totals[r.p] = (totals[r.p] || 0) + r.bq;
    if (r.c !== "—") totals[r.c] = (totals[r.c] || 0) + r.bq;
  });
  const anon = {};
  Object.keys(totals).sort((a, b) => totals[b] - totals[a]).forEach((name, i) => { anon[name] = `Customer ${i + 1}`; });
  rows.forEach((r) => {
    if (r.p !== "—") r.p = anon[r.p];
    if (r.c !== "—") r.c = anon[r.c];
  });
...
export function parseWorkbook(arrayBuffer) {
  ...
  if (col.delv < 0) throw new Error("Prd Delv Dt column could not be found in the Excel file.");
