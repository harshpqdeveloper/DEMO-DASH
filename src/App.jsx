...
import PQIntro from "./PQIntro.jsx";
...
export default function App() {
  ...
  const loadLatest = useCallback(async () => {
    ...
    try {
      ...
      console.info(
        `[PQ Dash] Detected Excel file: ${metaJson.fileName}\n` +
        `[PQ Dash] Total rows (with Order SrNo): ${parsed.meta.linesAnyYear}\n` +
        `[PQ Dash] Date column used for year filter: ${parsed.meta.dateColumn}\n` +
        `[PQ Dash] ${parsed.meta.targetYear} rows: ${parsed.meta.totalLines}\n` +
        `[PQ Dash] Ignored non-${parsed.meta.targetYear} rows: ${parsed.meta.linesIgnored}`
      );
  ...
  const intro = !introDone && (
    <PQIntro ready={status !== "loading"} onFinish={() => setIntroDone(true)} />
  );
