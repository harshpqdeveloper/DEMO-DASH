...
export default function PQIntro({ ready, minDuration = DEFAULT_MIN_MS, onFinish }) {
  ...
  return (
    <div className={`pq-intro${exiting ? " pq-intro--exit" : ""}`} role="presentation" aria-hidden="true">
      <style>{INTRO_CSS}</style>
      <div className="pq-intro__particles">
        {PARTICLES.map((p, i) => (
          <span
            key={i}
            className={`pq-intro__particle pq-intro__particle--${p.shape}`}
            style={{
      ...
      <div className="pq-intro__brand">
        <PQLogo size="xl" />
        <span className="pq-intro__shimmer" />
      </div>
    </div>
  );
}
...
.pq-intro--exit{opacity:0;pointer-events:none}

.pq-intro__particles{position:absolute;inset:0;pointer-events:none}
.pq-intro__particle{
  position:absolute;border-radius:999px;
  animation-name:pqFloat;animation-timing-function:ease-in-out;animation-iteration-count:infinite;
  will-change:transform;
}
.pq-intro__particle--diamond{border-radius:3px;transform:rotate(45deg)}

.pq-intro__brand{
  position:relative;display:flex;align-items:center;justify-content:center;
  animation:pqBrandIn ${ENTER_MS}ms cubic-bezier(.16,1,.3,1) both;
  will-change:transform,opacity,filter;
  transition:transform ${EXIT_MS}ms cubic-bezier(.4,0,.2,1), opacity ${EXIT_MS}ms cubic-bezier(.4,0,.2,1);
}
.pq-intro--exit .pq-intro__brand{opacity:0;transform:scale(1.05)}
...
.pq-intro__shimmer{
  ...
  animation:pqShimmer 900ms ease-out ${ENTER_MS - 150}ms 1 both;
  pointer-events:none;
}
...
.pq-intro__particle--diamond{animation-name:pqFloatDiamond}
@keyframes pqFloatDiamond{
  0%,100%{transform:translateY(0) rotate(45deg)}
  50%{transform:translateY(-12px) rotate(45deg)}
}

@media (prefers-reduced-motion: reduce){
  .pq-intro__brand{animation:none;opacity:1;filter:none;transform:none}
  .pq-intro__shimmer{animation:none;opacity:0}
  .pq-intro__particle{animation:none}
  .pq-intro{transition:none}
}
`;
