# PQ Dash — Production &amp; Casting Dashboard
...
1. Unzip this project somewhere permanent, e.g. `C:\pq-dash`.
...
```
netsh advfirewall firewall add rule name="PQ Dash 8080" dir=in action=allow protocol=TCP localport=8080
```
...
```
npm install -g pm2
pm2 start npm --name pq-dash -- run preview
pm2 save
pm2 startup      # follow the printed instruction so it auto-starts on boot
```
Manage it with `pm2 status`, `pm2 restart pq-dash`, `pm2 logs`.
